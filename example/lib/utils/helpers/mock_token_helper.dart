...
class MockTokenHelper {
  /// Constructor for [MockTokenHelper]
  ...
  static Token buildFollowToken(
    ...
    FeedId? feed,
      ...
      _buildBackendToken(
          secret, TokenResource.follower, action, feed?.followClaim ?? '*');
...
class MockTokenGenerator {
  final String secretKey = MockStreamFeedConstants.secretKey;
  final String feedName = MockStreamFeedConstants.feedName;
  final String username = mockCurrentUsername;
  final String globalUsername = MockStreamFeedConstants.globalUsername;

  String get getMockOpenGraphToken =>
      MockTokenHelper.buildOpenGraphToken(secretKey).token;

  String get getMockUserReadToken =>
      MockTokenHelper.buildUsersToken(secretKey, TokenAction.read).token;

  String get getMockUserWriteToken =>
      MockTokenHelper.buildUsersToken(secretKey, TokenAction.write).token;

  String get getMockFeedWriteToken => MockTokenHelper.buildFeedToken(
          secretKey, TokenAction.write, FeedId(feedName, username))
      .token;

  String get getMockFeedReadToken => MockTokenHelper.buildFeedToken(
          secretKey, TokenAction.read, FeedId(feedName, globalUsername))
      .token;

  String get getMockFollowFeedWriteToken => MockTokenHelper.buildFollowToken(
      secretKey,
      TokenAction.write,
      FeedId(
        MockStreamFeedConstants.feedName,
        MockStreamFeedConstants.globalUsername,
      )).token;
}
